...
for i in xrange(0, len(contours)):
    ...
    if cv2.mean(hsv_image, temp_mask)[1] > 190 or \
                    cv2.mean(hsv_image, temp_mask)[2] > 160:
        continue
    print cv2.mean(hsv_image, temp_mask)
    res = cv2.bitwise_and(temp_img, temp_img, mask=temp_mask)
    new_contours.append(contours[i])
    # font = cv2.FONT_HERSHEY_SIMPLEX
    # #cv2.putText(temp_img,'Frame Number %s/256' %(len(img_list)-1),
    #               (10,100), font, 1,(255,255,0),2)
    # cv2.putText(res,'Eggs found %s' %(len(contours)),(500,100),
    #             font, 1,(255,255,0),2)
...
# thresh = cv2.adaptiveThreshold(hsv_image[:, :, 2], 255,
